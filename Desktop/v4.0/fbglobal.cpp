...
bool is_vertex(FBArray3D<unsigned char> &bvfmap, long i1, long i2, long i3) 
{
 ...
 for (int d3=-1; d3<=0; d3++)
  if (is_element(bvfmap, i1+d1, i2+d2, i3+d3)) return true;
 return false;
}


//-----------------------------------------------------------------------------
fbreal initial_displacement(int i1,int i2,int i3, int dir,fbreal resolution[3], 
                            FBMacroscopicStrain &macroscopic_strain) 
{
 // Returns one displacement (one vertex, one direction) in float units (mm)
